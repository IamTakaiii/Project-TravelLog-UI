...
import { fundsQueryOptions, fundsQueryKeys, useFundMutations } from "@/features/money/queries/fund-queries";
...
export function CentralFundSheet({
    ...
    const { createMutation, updateMutation, deleteMutation } = useFundMutations(tripId);
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            {funds.length === 0 ? (
                                ...
                                funds.map((fund) => {
                                    const details = summary?.fundDetails?.find(f => f.id === fund.id);
                                                ...
                                                <div className="flex items-start gap-4 min-w-0">
                                                    ...
                                                    <div className="space-y-1 min-w-0">
                                                        ...
                                                        <div className="flex items-baseline gap-1.5 flex-wrap">
                                                            ...
                                                            <span className={`font-bold font-mono text-sm ${isDepleted ? 'text-destructive' : 'text-primary'}`}>
                                                            ...
                                                            <span className="text-xs font-medium text-muted-foreground/60 ml-1">
                                                                / {fund.amount.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
