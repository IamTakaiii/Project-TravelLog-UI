...
function ModernStatsCard({ title, value, trend, icon: Icon, color, bgColor }: StatsCardProps) {
    return (
        <Card className="border-border/40 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 bg-background/60 backdrop-blur-sm group cursor-default">
            <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                    ...
                    {trend && <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground bg-muted px-2 py-1 rounded-full">{trend}</span>}
                </div>
                <div>
                    <div className="text-3xl font-[800] tracking-tight mb-1 group-hover:text-primary transition-colors">{value}</div>
